...
#
# KV_OFFLOAD selects the tier topology:
#   none    -> no offloading (default)
#   cpu     -> CPU RAM tier only (GPU -> host RAM)
#   fs      -> CPU RAM + filesystem tier (GPU -> host RAM -> local NVMe/disk).
#              OFFLOAD_DISK_PATH must be set when KV_OFFLOAD=fs.
#
...
#
# A per-host subdir is appended to OFFLOAD_DISK_PATH so prefill and decode nodes
# don't collide on a shared mount.
#
# Env: KV_OFFLOAD        = none (default) | cpu | fs
#      OFFLOAD_BACKEND   = native (default) | lmcache        (only read when KV_OFFLOAD!=none)
...
kv_offload_fs_enabled() {
    [[ "${KV_OFFLOAD:-none}" == "fs" ]]
}

# Per-host dir for the filesystem tier. Only meaningful when KV_OFFLOAD=fs.
# The per-host subdir keeps prefill and decode nodes from colliding on a shared mount.
_kv_offload_fs_dir() {
    kv_offload_fs_enabled || return 0
...
_kv_offload_validate() {
    case "${KV_OFFLOAD}" in
        none|cpu|fs) ;;
        *)
            echo "Error: unsupported KV_OFFLOAD='${KV_OFFLOAD}' (expected none|cpu|fs)." >&2
            exit 1
            ;;
    esac
    ...
    if kv_offload_fs_enabled && [[ -z "${OFFLOAD_DISK_PATH:-}" ]]; then
        echo "Error: KV_OFFLOAD=fs requires OFFLOAD_DISK_PATH to be set." >&2
        exit 1
    fi
}
...
# the filesystem-tier dir when KV_OFFLOAD=fs. Submit-time exports win.
kv_offload_setup_env() {
    kv_offload_enabled || return 0

    local disk_dir=""
    if kv_offload_fs_enabled; then
    ...
    # Disk tier: LMCache spills CPU-evicted chunks here (LRU, per-GPU sharded).
    [[ -n "${disk_dir}" ]] || return 0
