...
# KV_OFFLOAD overlay (orthogonal axis): cpu/fs attach a CPU-RAM (and optionally fs) tier
# via MultiConnector. OFFLOAD_BACKEND selects the CPU tier's backend (native | lmcache);
# only meaningful when KV_OFFLOAD != none.
KV_OFFLOAD="${KV_OFFLOAD:-none}"
case "$KV_OFFLOAD" in none|cpu|fs) ;; *) echo "Error: invalid KV_OFFLOAD='${KV_OFFLOAD}' (expected none|cpu|fs)." >&2; exit 1 ;; esac
